...
export class Ed25519KeyPair implements KeyPair {
    ...
    static async fingerprintFromPublicKey(importableType: JsonWebKey2020) : Promise<string> {
      const { publicKey } = await Ed25519KeyPair.from(importableType);
      return getMultibaseFingerprintFromPublicKeyBytes(publicKey);
    }
  
    static async toX25519KeyPair(kp: Ed25519KeyPair) : Promise<X25519KeyPair> {
    ...
    static generate = async ({secureRandom,}: {secureRandom: () => Uint8Array;}) : Promise<Ed25519KeyPair> => {
    ...
    static from = async (k: JsonWebKey2020 | Ed25519VerificationKey2018) : Promise<Ed25519KeyPair> => {
    ...
    static async fromFingerprint({ fingerprint }: { fingerprint: string }):Promise<Ed25519KeyPair> {
